...
CREATE TABLE ADV_INFO(
    ...
    ADV_TITLE VARCHAR2(255 CHAR) NOT NULL,
    ADV_CATEGORY VARCHAR2(20 CHAR) NOT NULL,
    ADV_DESC VARCHAR2(255 CHAR),
    ADV_LINK VARCHAR2(255 CHAR),
    ADV_IMAGE_NAME VARCHAR2(255 CHAR),
    ADV_IMAGE_PATH VARCHAR2(255 CHAR),
...
COMMENT ON COLUMN ADV_INFO.ADV_TITLE IS '광고 제목';
COMMENT ON COLUMN ADV_INFO.ADV_CATEGORY IS '광고 카테고리';
COMMENT ON COLUMN ADV_INFO.ADV_DESC IS '광고 설명';
COMMENT ON COLUMN ADV_INFO.ADV_LINK IS '광고 링크';
COMMENT ON COLUMN ADV_INFO.ADV_IMAGE_NAME IS '광고 이미지 파일명';
COMMENT ON COLUMN ADV_INFO.ADV_IMAGE_PATH IS '광고 이미지 경로';
...
CREATE SEQUENCE ADV_SEQ START WITH 1 INCREMENT BY 1 NOCACHE;

CREATE OR REPLACE TRIGGER TRG_ADV_INFO_REG
    BEFORE INSERT ON ADV_INFO
    FOR EACH ROW
BEGIN
    SELECT ADV_SEQ.NEXTVAL INTO :NEW.ADV_ID FROM DUAL;
